...
export class ScheduleManager {
    ...
    async addItemToSchedule(day: number, hour: number, item: Omit<SchedulerItem, 'id'>) {
        ...
        if (!weekData.schedule[day]) weekData.schedule[day] = {};
        if (!weekData.schedule[day][hour]) weekData.schedule[day][hour] = [];
    ...
    private syncDeadlineIntoWeekly(item: SchedulerItem) {
        if (!this.currentYearData || !item.deadlineDate || item.deadlineHour == null) return;
        ...
        if (weekYear !== this.currentYearData.year) return;
        ...
        if (!weekData.schedule[dayOfWeek]) weekData.schedule[dayOfWeek] = {};
        if (!weekData.schedule[dayOfWeek][hour]) weekData.schedule[dayOfWeek][hour] = [];
    ...
    async removeItem(
        ...
        if (!this.currentYearData) {
            return { needsSaveBacklog: false, needsSaveGoals: false, needsSaveYear: false };
        }
        ...
        for (const weekData of this.currentYearData.weeks) {
            for (const day in weekData.schedule) {
                for (const hour in weekData.schedule[day]) {
                    ...
                    if (weekData.schedule[day][hour].length < originalLength) needsSaveYear = true;
                }
            }
        }
        ...
        for (const month in this.currentYearData.monthlyTasks) {
            ...
            if (this.currentYearData.monthlyTasks[month].length < originalLength) needsSaveYear = true;
        }

        const newGoals = generalGoals.filter(item => item.id !== itemId);
        if (newGoals.length < generalGoals.length) needsSaveGoals = true;

        const newBacklog = backlogItems.filter(item => item.id !== itemId);
        if (newBacklog.length < backlogItems.length) needsSaveBacklog = true;
    ...
    async clearMonthTasks(month: number) {
        ...
        await this.saveYearData();
        this.refreshView();
        this.onNotice('Month tasks cleared!');
    }
    ...
    async clearAllTasks() {
        ...
        await this.saveYearData();
        this.refreshView();
        this.onNotice(`Cleared all ${cleared} tasks!`);
    }
